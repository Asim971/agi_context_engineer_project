...
var DatabaseService = class DatabaseService extends BaseService {
  ...
  constructor(isTest = false) {
    ...
    if (typeof BaseService === "undefined") {
      throw new Error(
        "BaseService must be loaded before DatabaseService. Check file loading order."
      );
    }
    ...
    this.executeWithErrorHandlingSync(
      () => {
        ...
        if (this.isTest) {
          ...
          if (!spreadsheetId) {
            this.logger.warn(
              "Test spreadsheet ID not found in properties. A new one may be created by TestEnvironmentService."
            );
        ...
        try {
          ...
          throw new AppScriptError(
            ...
            `Failed to open spreadsheet with ID: ${spreadsheetId}`,
            { error: e.message }
          );
        }
      },
  ...
  getSheet(sheetName) {
    return this.executeWithErrorHandlingSync(
      () => {
        if (!this.spreadsheet) {
          this.logger.warn(
            "Spreadsheet not initialized, attempting re-initialization."
          );
          this.initialize();
          if (!this.spreadsheet) {
            throw new AppScriptError(
              "SPREADSHEET_UNAVAILABLE",
              "Spreadsheet is not available."
            );
          }
        }
        ...
        if (!sheet) {
          this.logger.info("Sheet not found, creating new sheet.", {
            sheetName,
          });
  ...
  getHeadersForSheet(sheetName) {
    ...
    return (
      Config.HEADERS[
        Object.keys(Config.SHEETS).find(
          (key) => Config.SHEETS[key] === sheetName
        )
      ] || []
    );
  }
  ...
  updateRecord(sheetName, keyField, keyValue, data) {
    return this.executeWithErrorHandlingSync(
      () => {
        ...
        if (keyIndex === -1) {
          throw new AppScriptError(
            "CONFIG_KEY_FIELD_NOT_FOUND",
            `Key field "${keyField}" not found in sheet "${sheetName}".`
          );
        }
          ...
          if (allRecords[i][keyIndex] == keyValue) {
            ...
            const rowData = headers.map((header) =>
              data[header] !== undefined
                ? data[header]
                : allRecords[i][headers.indexOf(header)]
            );
        ...
        this.logger.warn("Record not found for update.", {
          sheetName,
          keyField,
          keyValue,
        });
      ...
      { sheetName, keyField, keyValue, data },
      "updateRecord"
    );
  }
  ...
  findRecords(sheetName, criteria) {
    return this.executeWithErrorHandlingSync(
      () => {
        ...
        this.logger.debug(
          `Found ${filtered.length} records matching criteria.`,
          { sheetName, criteria }
        );
        return filtered;
      },
...
// Make DatabaseService globally available for ServiceBootstrap
...
console.log("DatabaseService registered globally");

// DatabaseService and getGlobalDB are now managed via GlobalServiceLocator for dependency injection
// Global assignments restored for ServiceBootstrap compatibility
